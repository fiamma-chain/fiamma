...
for i in "${!nodes[@]}"; do 
    echo $(pwd)
    seeds=$(docker run -v "$(pwd)/testnet-nodes/${nodes[$i]}:/root/.fiamma:Z" -it ghcr.io/fiamma-chain/fiamma  config get config p2p.persistent_peers)
    for j in "${!nodes[@]}"; do  
        seeds=${seeds//${nodes[$j]}/${nodes_ips[$j]}}
    done
    
    docker run -v "$(pwd)/testnet-nodes/${nodes[$i]}:/root/.fiamma:Z" -it ghcr.io/fiamma-chain/fiamma  config set config p2p.persistent_peers $seeds --skip-validate    
done
